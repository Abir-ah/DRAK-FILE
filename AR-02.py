...
\x1b[93;1m Whatsapp     \x1b[92;1m=>      \x1b[93;1m01730853768
...
def menu(): #Bilo
	...
	if farhan in ['1','01']:
		...
		os.system("https://www.facebook.com/Abir.lg17289")
		exit()
...
def crack2(idf,pwv):
	...
	fff = '%'
	print('\r%s [AR03] %s/%s  OK*%s | CP*%s => %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s [AR02-CP] %s|%s        '%(b,idf,pw))
